...
int main()
{
    ...
    Coordinates::calibrateToRobotOrigo("../offset/*.bmp", calibrationMaps);
    ...
    try
    {
        ...
        while ( camera.IsGrabbing())
        {
            ...
            if (ptrGrabResult->GrabSucceeded())
            {
                ...
                cv::Mat table(openCvImage, cv::Rect(440,0,700, 880));
                cv::Mat table2D(table.rows, table.cols, table.type());


                cv::Mat grey(table2D.clone());
                cv::cvtColor(table2D, grey, cv::COLOR_BGR2GRAY);
                ...
                for( size_t i = 0; i < circles.size(); i++ )
                {
                    ...
                    // circle center
                    cv::circle( table2D, center, 1, cv::Scalar(0,100,100), 3, cv::LINE_AA);
                    // circle outline
                    int radius = c[2];
                    cv::circle( table2D, center, radius, cv::Scalar(255,0,255), 3, cv::LINE_AA);
                }
                ...
                if (circles.size() == 1) {
                    ...
                    cv::Vec3f angledCenterVec(c[0], c[1], 1);
                    cv::Vec3f CenterVec(c[0], c[1], 1);

                    cv::Mat perspective2D;

                    cv::Point2f* cameraP = Coordinates::generate4CameraPoints();
                    cv::Point2f* robotP = Coordinates::generate4RobotPoints();
...
//                    cv::warpPerspective(table, table2D, perspective2D, table2D.size());
                    ...
                    for (int i = 0 ; i < perspective2D.rows; i++) {
                        for (int j = 0; j < perspective2D.cols; j++) {
                            CenterVec[i] += static_cast<float>(perspective2D.at<double>(i,j)) * angledCenterVec(j);
                        }
                    }
